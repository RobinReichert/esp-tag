...
impl WireCodec<MESSAGE_SIZE> for MessageContent {
    ...
    fn encode(&self, out: &mut MessageData) -> Result<(), CodecError> {
        ...
        match self {
            Self::Application(d) => {
                out.push(d.len() as u8).map_err(|e| CodecError::BufferOverflowError(e))?;
                out.extend_from_slice(d).map_err(|e| CodecError::BufferCapacityError(e))?;
            },
            Self::Discovery => {},
            Self::Invitation => {},
            Self::RequestNews => {},
    ...
    fn decode(cursor: &mut Cursor<'_>) -> Result<Self, CodecError> {
        ...
        let msg_type = MessageType::try_from(type_byte).map_err(|e| CodecError::MessageTypeError(e))?;
        match msg_type {
            MessageType::Application => {
                let len_byte = cursor.take(1).map_err(|e| CodecError::CursorReadError(e))?[0];
                let mut d = MessageData::new();
                d.extend_from_slice(cursor.take(len_byte as usize).map_err(|e| CodecError::CursorReadError(e))?).map_err(|e|CodecError::BufferCapacityError(e))?;
...
impl SendMessage {
    ...
    pub fn serialize(&self) -> Result<MessageData, SendMessageError> {
        ...
        self.final_source.encode(&mut out).map_err(|e| SendMessageError::FinalSourceEncodeError(e))?;
        Ok(out)
    }
}
