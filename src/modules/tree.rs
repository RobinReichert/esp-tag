...
use heapless::Vec;
use core::{ result::Result, option::Option, cell::RefCell};
...
impl Tree {
    ...
    fn remove_node_helper(&self, address: Node, current_id: SlotId) -> Option<SlotId> {
        ...
        if let Some(pos) = current.nexts.iter().enumerate().find_map(|(idx, next_id)|{
            ...
            if next.node == address { Some(idx) } else { None }
        }) {
            return Some(current.nexts.remove(pos)); 
        }
        if let Some(res) = current.nexts.iter().find_map(|next_id| {
            self.remove_node_helper(address, *next_id)
        }) {
            return Some(res);
        }
        None
    ...
    fn insert_node_helper(&self, parent_address: Node, current_id: SlotId, leaf_id: SlotId) -> Option<()> {
        let mut current = self.leafs.get(current_id).ok()?.borrow_mut();
        ...
        if let Some(res) = current.nexts.iter().find_map(|next_id| {
            self.insert_node_helper(parent_address, *next_id, leaf_id)
        }) {
            return Some(res);
        }
        None
    ...
    fn next_hop_helper(&self, destination: Node, current_id: SlotId) -> Result<Node, TreeError> {
        ...
        if let Some(&ret_id) = current.nexts.iter()
            .find(|&&next_id| {
...
impl<T, const N: usize> Arena<T, N> {
    ...
    fn new() -> Self {
        Arena { free: (0..N as SlotId).collect(), slots: [(); N].map(|_| None ) }
    }
